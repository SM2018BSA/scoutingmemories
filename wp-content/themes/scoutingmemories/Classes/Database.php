...
class Database {
	...
	private static function generate_cache_key( $where, $args, $field, $type ) {
		$cache_key = '';
		$where     = AppHelper::array_flatten( $where );
	...
	public static function get_associative_array_results( $columns, $table, $where ) {
		...
		$cache_key = str_replace( array(
			' ',
			','
		), '_', trim( implode( '_', AppHelper::array_flatten( $where ) ) . $columns . '_results_ARRAY_A', ' WHERE' ) );
	...
	public static function save_settings( $settings, $group ) {
		...
		$settings['post_content'] = AppHelper::prepare_and_encode( $settings['post_content'] );
	...
	public static function check_cache( $cache_key, $group = '', $query = '', $type = 'get_var', $time = 300 ) {
		...
		if ( ! AppHelper::is_empty_value( $results, false ) || empty( $query ) ) {
			return $results;
		}
