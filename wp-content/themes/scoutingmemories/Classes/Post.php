...
class Post {
	...
	public function __construct( $post_id ) {
		...
		if (array_key_exists('state',   $this->the_post_meta)) { $this->state_val = maybe_unserialize( $this->the_post_meta['state'][0]);   }
		if (array_key_exists('council', $this->the_post_meta)) { $council_keys = maybe_unserialize( $this->the_post_meta['council'][0]); }
		if (array_key_exists('lodge',   $this->the_post_meta)) { $this->lodge  = maybe_unserialize( $this->the_post_meta['lodge'][0]);   }
		if (array_key_exists('camp',    $this->the_post_meta)) { $this->camp   = maybe_unserialize( $this->the_post_meta['camp'][0]);    }
