...
class Theme {
    ...
    // Front-end library versions (loaded from cdn.jsdelivr.net). Update here to upgrade.
    const BOOTSTRAP_VERSION       = '5.3.8';   // bootstrap.bundle includes Popper
    const BOOTSTRAP_ICONS_VERSION = '1.13.1';
    const SELECT2_VERSION         = '4.1.0';
    ...
    public function __construct() {
	    ...
	    $this->addScript('bootstrap', 'https://cdn.jsdelivr.net/npm/bootstrap@' . self::BOOTSTRAP_VERSION . '/dist/js/bootstrap.bundle.min.js', array() ,false );

		//$theme->addScript('bs-multiselect', '/js/BsMultiSelect.min.js');
	    $this->addScript('select2', 'https://cdn.jsdelivr.net/npm/select2@' . self::SELECT2_VERSION . '/dist/js/select2.min.js', '', false);



	    //$this->addStyle('bootstrap', '/css/bootstrap.min.css' );
	    $this->addStyle('bootstrap', 'https://cdn.jsdelivr.net/npm/bootstrap@' . self::BOOTSTRAP_VERSION . '/dist/css/bootstrap.min.css','outside' );
	    $this->addStyle('bootstrap-icons', 'https://cdn.jsdelivr.net/npm/bootstrap-icons@' . self::BOOTSTRAP_ICONS_VERSION . '/font/bootstrap-icons.min.css','outside' );
...
/* -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- */
    ...
    public function addScript($handle, $src, $deps=array(), $file_uri=true) {
        $this->actionEnqueueScripts(function() use ($handle, $src, $deps, $file_uri) {
            if ($file_uri)
                wp_enqueue_script($handle, get_theme_file_uri() . $src, $deps, self::fileVersion($src));
            else
	            wp_enqueue_script($handle, $src, $deps, null);
        });
    }

	// Cache-busting version for theme files: changes only when the file changes, so browsers
	// can keep cached copies between page loads. CDN URLs already contain their version.
	private static function fileVersion($src) {
    ...
    public function addStyle($handle, $src, $uri = null) {
        $this->actionEnqueueScripts(function() use ( $uri, $handle, $src) {
            if ($uri) { wp_enqueue_style($handle, $src, array(), null); }
            else { wp_enqueue_style($handle, get_theme_file_uri(). $src, array(), self::fileVersion($src)); }
     ...
     *     ?>
     */
    public static function bootstrap_pagination( $the_query = null, $echo = true, $params = [] ) {
        ...
        if (isset($_GET[ 'state' ])) {
            $add_args[ 'state' ] = get_request_parameter('state' );
        }
        if (isset($_GET[ 'council' ])) {
            $add_args[ 'council' ] = get_request_parameter ('council' );
        }
        if (isset($_GET[ 'lodge' ])) {
            $add_args[ 'lodge' ] = get_request_parameter ( 'lodge' );
        }
        if (isset($_GET[ 'camp' ])) {
            $add_args[ 'camp' ] = get_request_parameter( 'camp' );
        }
        if (isset($_GET[ 'start_date' ])) {
            $add_args[ 'start_date' ] =  get_request_parameter('start_date' );
        }
        if (isset($_GET[ 'end_date' ])) {
            $add_args[ 'end_date' ] = get_request_parameter( 'end_date' );
        }
        ...
        if ( is_array( $pages ) ) {
            ...
            $pagination = '<nav class="pt-5" aria-label="Page navigation"><ul class="pagination">';
