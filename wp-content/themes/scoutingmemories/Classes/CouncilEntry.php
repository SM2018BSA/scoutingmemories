...
class CouncilEntry extends Entry {
	...
	public function __construct(int $entry_id = 0, $merged_councils = array(), $get_merged = TRUE) {
		...
		if ($entry_id && !is_string($entry_id)) {
			...
			$this->number = array_key_exists('council_num', $this->entry_array) ? $this->entry_array['council_num'] : '';
			$this->name = array_key_exists('council_name', $this->entry_array) ? $this->entry_array['council_name'] : '';
			$this->state = array_key_exists('council_state', $this->entry_array) ? $this->entry_array['council_state'] : '';
			$this->state_val = array_key_exists('council_state-value', $this->entry_array) ? $this->entry_array['council_state-value'] : '';
			$this->council_slug = array_key_exists('council_slug', $this->entry_array) ? $this->entry_array['council_slug'] : '';
			$this->start_date = array_key_exists('council_start', $this->entry_array) ? $this->entry_array['council_start'] : '';
			$this->end_date = array_key_exists('council_end', $this->entry_array) ? $this->entry_array['council_end'] : '';
			$this->active = array_key_exists('council_active_1', $this->entry_array) ? $this->entry_array['council_active_1'] : '';
	...
	public static function get_council_name($council_slug) {
		...
		if (isset($entry->entry_array) && is_array($entry->entry_array) && array_key_exists('council_name', $entry->entry_array)) {
	...
	public static function get_council_number($council_slug) {
		$entry_id = Entry::get_field_id_from_key($council_slug);
		$entry = new Entry($entry_id);
		if (isset($entry->entry_array) && is_array($entry->entry_array) && array_key_exists('council_num', $entry->entry_array)) {
