    ...
    <div id="primary" class="content-area container mt-5 h-100 aasdf" >
		...
		<?php
            if (($the_user && $the_user->cap_allowed('index_contributor')) || current_user_can('edit_others_posts')) {
                ...
                if (shortcode_exists('sm_add_camp_form')) {
                    echo do_shortcode('[sm_add_camp_form]');
                } elseif (isset($AddCampForm)) {
                    echo $AddCampForm->show_form();
                }
            } else {
                echo '<div class="alert alert-warning">' . esc_html__('You do not have permissions to add a camp.', 'scouting-memories-2026') . '</div>';
            }
		?>
