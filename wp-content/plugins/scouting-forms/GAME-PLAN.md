...
- 2026-09-25: Archive Tools "update end dates" now accepts only a real year (it writes into every active council/camp/lodge). Incident: my test of that check included a value that cleans to a valid year, so the tool ran locally and set 609 active end dates to 2026; restored from E:/WEB/wp_live_backup.sql and verified. A full comparison of the Formidable tables with that backup then showed no other changes from testing (entries/values/fields/forms); the only remaining differences are accented and non-Latin characters shown as "?" in the local copy, left by the original import (entry dates unchanged, live unaffected).
