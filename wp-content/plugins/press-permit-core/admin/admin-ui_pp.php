...
class PP_AdminUI
{
	...
	function build_menu() {
		...
		if ( 'pp-groups' == $pp_cred_menu )  {
			...
			$menu_img = (defined('PUBLISHPRESS_VERSION') && version_compare(PUBLISHPRESS_VERSION, '1.20.2', '>')) ? '' : PP_URLPATH . '/admin/images/menu/users-c.png';
			add_menu_page( __('Permissions', 'pp'), __('Permissions', 'pp'), 'read', $pp_cred_menu, array(&$this, 'menu_handler'), $menu_img, $pp_cred_key );
		}
